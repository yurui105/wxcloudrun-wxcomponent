...
func TemplateMessageSetIndustry(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.SetIndustry(c.Request.Context(), industryId1, industryId2, nil)
	if err != nil {
		panic(err)
	}
...
func TemplateMessageGetIndustry(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.GetIndustry(c.Request.Context())
	if err != nil {
		panic(err)
	}
...
//		return
//	}
//	rs, err := app.TemplateMessage.AddTemplate(c.Request.Context(), shortID)
//	if err != nil {
//		panic(err)
//	}
//
...
func GetPrivateTemplates(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.GetPrivateTemplates(c.Request.Context())
	if err != nil {
		panic(err)
	}
...
func DelPrivateTemplate(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.DeletePrivateTemplate(c.Request.Context(), templateID)
	if err != nil {
		panic(err)
	}
...
func TemplateMessageSend(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.Send(c.Request.Context(), &request.RequestTemlateMessage{
		ToUser:     toUser,
...
func SendSubscribe(c *gin.Context) {
	...
	rs, err := app.TemplateMessage.SendSubscription(
		c.Request.Context(),
